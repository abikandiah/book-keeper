...
```
scripts/generate-sandboxed.sh "Book Title" [--force] [--notes ./my-notes.txt]
```

This builds `Dockerfile.generate` — a minimal image containing only
...
network access of its own — to re-validate that JSON and do the actual
`book/<slug>` branch-create-and-commit (the same logic `pnpm run generate`
uses directly, now shared via `scripts/lib/publish.ts`). The rest of the
workflow (steps 3-6 above) is unchanged either way.
